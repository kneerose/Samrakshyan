    ...
    return (
        <div className="flex flex-col w-full space-y-10 items-center justify-center pt-10">
            <div className="flex justify-center">
                <input type="file" accept="audio/*" className="opacity-0 w-0 h-2" ref={(fileinput) => (fileInput = fileinput)} onChange={createWaveform} />
                <Button children="Import Sound" onClick={() => fileInput.click()} className="bg-buttonColor text-white" />
            </div>
            <AudioPlayerCard waveform={waveform} setPosition={setPosition} currentPosition={currentPosition} setAudio={setAudio} isAudio={isAudio} waveformComponent={getWaveformComponent()} file={file} />
