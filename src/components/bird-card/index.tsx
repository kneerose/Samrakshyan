...
export default function BirdCard({ birdDetail }: IBirdCardProps) {
  ...
  return (
    <div
      aria-hidden
      className="bg-white  relative my-4 dark:bg-neutral-800 flex justify-between flex-col shadow-md sm:hover:scale-100 lg:hover:scale-105 rounded-lg overflow-hidden transition duration-500"
      data-aos="fade-up"
    >
      <div
      ...
      <div className=" flex bg-white absolute bottom-0 items-center justify-between px-6 left-0 space-x-4 right-0 text-center">
        ...
        {isPlaying ? (
          <SpeakerWave
            onClick={pauseAudio}
            className={cn("h-5 w-5 cursor-pointer text-blue-800")}
          />
        ) : (
          <SpeakerXMark
            onClick={startAudio}
            className={cn("h-5 w-5 cursor-pointer text-lightBlackColor")}
          />
        )}
      </div>
    </div>
